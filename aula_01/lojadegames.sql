...
-- relacionar tres tabelas
-- começa com a tabela que tem as chaves estrangeiras
-- e faz um inner join com cada tabela que ela tem relação
-- relação unidirecional, 
SELECT * FROM tb_produtos INNER JOIN tb_categorias ON tb_produtos.categoria_id = tb_categorias.id INNER JOIN td_usuarios ON td_produtos.usuario_id = tb_usuarios.id
